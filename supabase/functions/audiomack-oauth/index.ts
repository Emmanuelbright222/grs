...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Handle both GET (from Audiomack redirect) and POST (from frontend callback)
    ...
    if (error) {
      console.error("Audiomack OAuth error:", error);
      return new Response(
    ...
    let redirectUri = Deno.env.get("AUDIOMACK_REDIRECT_URI");
    if (!redirectUri) {
      const frontendUrl = Deno.env.get("FRONTEND_URL");
      if (!frontendUrl) {
        console.error("AUDIOMACK_REDIRECT_URI or FRONTEND_URL must be set");
        return new Response(
          JSON.stringify({ error: "Redirect URI not configured" }),
          { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      }
      redirectUri = `${frontendUrl}/auth/audiomack/callback`;
    }
